...
namespace cb0t.Scripting.Statics
{
    ...
    class JSRegistry : ObjectInstance
    {
        ...
        public static bool Exists(ScriptEngine eng, String key)
        {
            ...
            RegistryKey rk = Registry.CurrentUser.OpenSubKey("Software\\cb0tv3\\scripting", false);

            if (rk == null)
                return false;

            rk.Close();
            rk = Registry.CurrentUser.OpenSubKey("Software\\cb0tv3\\scripting\\" + script, false);
        ...
        public static bool SetValue(ScriptEngine eng, String key, object value)
        {
            ...
            if (rk == null)
                Registry.CurrentUser.CreateSubKey("Software\\cb0tv3\\scripting");
            else
                rk.Close();

            rk = Registry.CurrentUser.OpenSubKey("Software\\cb0tv3\\scripting\\" + script, true);

            if (rk == null)
            {
                Registry.CurrentUser.CreateSubKey("Software\\cb0tv3\\scripting\\" + script);
                rk = Registry.CurrentUser.OpenSubKey("Software\\cb0tv3\\scripting\\" + script, true);
            }
        ...
        {
            String script = eng.ScriptName;
            RegistryKey rk = Registry.CurrentUser.OpenSubKey("Software\\cb0tv3\\scripting", true);
